...
type StoryEdges struct {
	...
	Views []*StoryView `json:"views,omitempty"`
...
func (e StoryEdges) ViewsOrErr() ([]*StoryView, error) {
	if e.loadedTypes[1] {
		return e.Views, nil
	}
...
func (*Story) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case story.FieldType, story.FieldMime, story.FieldPath, story.FieldRemoteURL, story.FieldMediaURL, story.FieldCdnURL, story.FieldStory, story.FieldName, story.FieldObjectID, story.FieldObjectURI, story.FieldBearcapToken:
	...
	for i := range columns {
		...
		case story.FieldCommentCount:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.CommentCount = new(uint)
				*_m.CommentCount = uint(value.Int64)
			}
...
// QueryViews queries the "views" edge of the Story entity.
func (_m *Story) QueryViews() *StoryViewQuery {
	return NewStoryClient(_m.config).QueryViews(_m)
}
...
func (_m *Story) String() string {
	...
	builder.WriteString(", ")
	if v := _m.ObjectURI; v != nil {
		builder.WriteString("object_uri=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")
	if v := _m.BearcapToken; v != nil {
