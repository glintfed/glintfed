...
func (_q *LikeQuery) QueryActor() *ProfileQuery {
...
func (_q *LikeQuery) QueryStatus() *StatusQuery {
...
func (_q *LikeQuery) WithActor(opts ...func(*ProfileQuery)) *LikeQuery {
	query := (&ProfileClient{config: _q.config}).Query()
...
func (_q *LikeQuery) WithStatus(opts ...func(*StatusQuery)) *LikeQuery {
	query := (&StatusClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LikeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Like, error) {
	...
	if query := _q.withActor; query != nil {
		if err := _q.loadActor(ctx, query, nodes, nil,
			func(n *Like, e *Profile) { n.Edges.Actor = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withStatus; query != nil {
		if err := _q.loadStatus(ctx, query, nodes, nil,
			func(n *Like, e *Status) { n.Edges.Status = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LikeQuery) loadActor(ctx context.Context, query *ProfileQuery, nodes []*Like, init func(*Like), assign func(*Like, *Profile)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Like)
	for i := range nodes {
...
func (_q *LikeQuery) loadStatus(ctx context.Context, query *StatusQuery, nodes []*Like, init func(*Like), assign func(*Like, *Status)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Like)
	for i := range nodes {
		fk := nodes[i].StatusID
...
func (_q *LikeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withActor != nil {
			_spec.Node.AddColumnOnce(like.FieldProfileID)
		}
		if _q.withStatus != nil {
			_spec.Node.AddColumnOnce(like.FieldStatusID)
		}
	}
