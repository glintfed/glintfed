...
type Avatar struct {
	...
	ChangeCount uint `json:"change_count,omitempty"`
...
func (e AvatarEdges) ProfileOrErr() (*Profile, error) {
	if e.Profile != nil {
	...
	for i := range columns {
		...
		case avatar.FieldChangeCount:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.ChangeCount = uint(value.Int64)
			}
...
// QueryProfile queries the "profile" edge of the Avatar entity.
func (_m *Avatar) QueryProfile() *ProfileQuery {
	return NewAvatarClient(_m.config).QueryProfile(_m)
}
