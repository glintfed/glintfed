...
func (_q *StoryQuery) QueryViews() *StoryViewQuery {
	query := (&StoryViewClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *StoryQuery) WithViews(opts ...func(*StoryViewQuery)) *StoryQuery {
	query := (&StoryViewClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *StoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Story, error) {
	...
	if query := _q.withViews; query != nil {
		if err := _q.loadViews(ctx, query, nodes,
			func(n *Story) { n.Edges.Views = []*StoryView{} },
			func(n *Story, e *StoryView) { n.Edges.Views = append(n.Edges.Views, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *StoryQuery) loadViews(ctx context.Context, query *StoryViewQuery, nodes []*Story, init func(*Story), assign func(*Story, *StoryView)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(storyview.FieldStoryID)
	}
	query.Where(predicate.StoryView(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(story.ViewsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
