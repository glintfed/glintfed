...
type UserQuery struct {
	...
	withFilters       *UserFilterQuery
	withDevices       *UserDeviceQuery
	withAccountLog    *AccountLogQuery
	withInterstitials *AccountInterstitialQuery
...
func (_q *UserQuery) QueryFilters() *UserFilterQuery {
...
// QueryAccountLog chains the current query on the "accountLog" edge.
func (_q *UserQuery) QueryAccountLog() *AccountLogQuery {
	query := (&AccountLogClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryInterstitials chains the current query on the "interstitials" edge.
func (_q *UserQuery) QueryInterstitials() *AccountInterstitialQuery {
	query := (&AccountInterstitialClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAccountLog:    _q.withAccountLog.Clone(),
		withInterstitials: _q.withInterstitials.Clone(),
...
func (_q *UserQuery) WithFilters(opts ...func(*UserFilterQuery)) *UserQuery {
	query := (&UserFilterClient{config: _q.config}).Query()
...
// WithAccountLog tells the query-builder to eager-load the nodes that are connected to
// the "accountLog" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAccountLog(opts ...func(*AccountLogQuery)) *UserQuery {
	query := (&AccountLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAccountLog = query
	return _q
}

// WithInterstitials tells the query-builder to eager-load the nodes that are connected to
// the "interstitials" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithInterstitials(opts ...func(*AccountInterstitialQuery)) *UserQuery {
	query := (&AccountInterstitialClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInterstitials = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [4]bool{
			_q.withFilters != nil,
			_q.withDevices != nil,
			_q.withAccountLog != nil,
			_q.withInterstitials != nil,
		}
	)
	...
	if query := _q.withFilters; query != nil {
		if err := _q.loadFilters(ctx, query, nodes,
			func(n *User) { n.Edges.Filters = []*UserFilter{} },
			func(n *User, e *UserFilter) { n.Edges.Filters = append(n.Edges.Filters, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAccountLog; query != nil {
		if err := _q.loadAccountLog(ctx, query, nodes,
			func(n *User) { n.Edges.AccountLog = []*AccountLog{} },
			func(n *User, e *AccountLog) { n.Edges.AccountLog = append(n.Edges.AccountLog, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withInterstitials; query != nil {
		if err := _q.loadInterstitials(ctx, query, nodes,
			func(n *User) { n.Edges.Interstitials = []*AccountInterstitial{} },
			func(n *User, e *AccountInterstitial) { n.Edges.Interstitials = append(n.Edges.Interstitials, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadFilters(ctx context.Context, query *UserFilterQuery, nodes []*User, init func(*User), assign func(*User, *UserFilter)) error {
	...
	for _, n := range neighbors {
		fk := n.user_filters
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_filters" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_filters" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadAccountLog(ctx context.Context, query *AccountLogQuery, nodes []*User, init func(*User), assign func(*User, *AccountLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountlog.FieldUserID)
	}
	query.Where(predicate.AccountLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AccountLogColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadInterstitials(ctx context.Context, query *AccountInterstitialQuery, nodes []*User, init func(*User), assign func(*User, *AccountInterstitial)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountinterstitial.FieldUserID)
	}
	query.Where(predicate.AccountInterstitial(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.InterstitialsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
