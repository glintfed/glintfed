...
func (_q *CommentQuery) QueryProfile() *ProfileQuery {
...
func (_q *CommentQuery) QueryStatus() *StatusQuery {
...
func (_q *CommentQuery) WithProfile(opts ...func(*ProfileQuery)) *CommentQuery {
	query := (&ProfileClient{config: _q.config}).Query()
...
func (_q *CommentQuery) WithStatus(opts ...func(*StatusQuery)) *CommentQuery {
	query := (&StatusClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := _q.withProfile; query != nil {
		if err := _q.loadProfile(ctx, query, nodes, nil,
			func(n *Comment, e *Profile) { n.Edges.Profile = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withStatus; query != nil {
		if err := _q.loadStatus(ctx, query, nodes, nil,
			func(n *Comment, e *Status) { n.Edges.Status = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CommentQuery) loadProfile(ctx context.Context, query *ProfileQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Profile)) error {
...
func (_q *CommentQuery) loadStatus(ctx context.Context, query *StatusQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Status)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Comment)
	for i := range nodes {
		fk := nodes[i].StatusID
...
func (_q *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProfile != nil {
			_spec.Node.AddColumnOnce(comment.FieldProfileID)
		}
		if _q.withStatus != nil {
			_spec.Node.AddColumnOnce(comment.FieldStatusID)
		}
	}
