...
func (_q *NotificationQuery) QueryActor() *ProfileQuery {
...
func (_q *NotificationQuery) QueryProfile() *ProfileQuery {
	query := (&ProfileClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *NotificationQuery) QueryStatus() *StatusQuery {
...
func (_q *NotificationQuery) WithActor(opts ...func(*ProfileQuery)) *NotificationQuery {
	query := (&ProfileClient{config: _q.config}).Query()
...
func (_q *NotificationQuery) WithProfile(opts ...func(*ProfileQuery)) *NotificationQuery {
	query := (&ProfileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NotificationQuery) WithStatus(opts ...func(*StatusQuery)) *NotificationQuery {
	query := (&StatusClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withActor; query != nil {
		if err := _q.loadActor(ctx, query, nodes, nil,
			func(n *Notification, e *Profile) { n.Edges.Actor = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withStatus; query != nil {
		if err := _q.loadStatus(ctx, query, nodes, nil,
			func(n *Notification, e *Status) { n.Edges.Status = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadActor(ctx context.Context, query *ProfileQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Profile)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Notification)
	for i := range nodes {
...
func (_q *NotificationQuery) loadProfile(ctx context.Context, query *ProfileQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Profile)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Notification)
	for i := range nodes {
...
func (_q *NotificationQuery) loadStatus(ctx context.Context, query *StatusQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Status)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Notification)
	for i := range nodes {
...
func (_q *NotificationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withActor != nil {
			_spec.Node.AddColumnOnce(notification.FieldActorID)
		}
		if _q.withProfile != nil {
			_spec.Node.AddColumnOnce(notification.FieldProfileID)
		}
		if _q.withStatus != nil {
			_spec.Node.AddColumnOnce(notification.FieldItemID)
		}
	}
